...
trained_filename = '20191222-022823_tx=20191221-213020_qbits=3_clipdb=0_snrdb=0-30_epochs=2500_lr=0.001_epsilon=0.1'
...
qbits = np.float(trained_filename.split('_')[2].split('=')[1])
clipdb = np.float(trained_filename.split('_')[3].split('=')[1])
...
for snrdb_idx, snrdb_val in enumerate(snrdb):
    ...
    channel = (np.random.normal(0, 1, (ofdm_size,1)) + np.random.normal(0, 1, (ofdm_size,1))) / np.sqrt(2)
